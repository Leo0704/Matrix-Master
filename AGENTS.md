...
**默认用户完全不懂编程。禁止使用任何专业黑话。**

- 不说术语：把"调用函数 / 编译 / 部署 / API / 依赖 / 路由 / ORM / token"全部换成日常说法或一句话解释。
- 每个新概念第一次出现时，用"它像什么"打个比方（做饭、寄快递、手机 App 之类）。
- 命令行、报错信息、配置项都要先翻译成人话，再说该怎么操作。
- 解释步骤时按"先做 A，再做 B，每步告诉你点哪个按钮、敲什么字、会出现什么画面"的顺序写。
- 如果非用专业词不可，必须紧跟一句大白话注释。
- 检验标准：让一个完全没写过代码的人读完你的回答，能照着做下去。

## 8. 只准用 Docker 跑

**所有运行动作（前端 / 后端 / 测试 / 数据库 / 任何服务）一律在 Docker 容器里执行，禁止污染本机环境。**

- 本机只允许装 Docker 和代码编辑器，其他什么 python、node、数据库、依赖包都不许装。
- 启动项目前先确认有 `Dockerfile` 或 `docker-compose.yml`；没有就先建，不准 fallback 到本机命令。
- 跑测试、跑服务、装依赖、跑迁移 —— 通通 `docker compose run` / `docker compose exec` / `docker run`。
- 遇到"在本机 pip install / npm install / 启动 xxx 服务"这类操作的请求，一律拒绝并改写成 docker 版本。
- 数据卷挂载只挂必要的代码目录，其他系统目录不许碰。
