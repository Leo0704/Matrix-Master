...
def _build_client(provider_name: str, *, base_url: str | None, **kwargs: Any) -> LLMClient:
    ...
    if spec.protocol == "anthropic":
        ...
        if "api_key" not in kwargs:
            kwargs["api_key"] = os.environ.get(spec.api_key_env)
...
def get_client(model_name: str, *, force_new: bool = False, **kwargs: Any) -> LLMClient:
    ...
    api_key = kwargs.get("api_key") or os.environ.get(spec.api_key_env) or ""
...
def get_default_client() -> LLMClient:
    ...
    """
    provider = os.environ.get("MATRIX_LLM_PROVIDER", "tongyi").lower()
    if provider not in PROVIDERS:
        raise ValueError(
            f"unsupported MATRIX_LLM_PROVIDER: {provider!r}; "
            f"choose from {sorted(PROVIDERS)}"
        )
    model = os.environ.get("MATRIX_LLM_MODEL", _default_model_for(provider))
